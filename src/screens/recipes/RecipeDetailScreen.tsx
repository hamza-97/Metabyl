...
import { ShoppingListItem } from '../../config/localLlmApi';

type RecipeDetailScreenRouteProp = RouteProp<RootStackParamList, 'RecipeDetail'>;

// Updated interface to match LLM response
interface LlmRecipeDetail {
  id: number; // Changed from string to number to match route params
  title: string;
  description: string;
  image?: string;
  ...
  instructions?: string;
  summary?: string;
  diets?: string[]; // Added to match existing code
}
...
const RecipeDetailScreen = () => {
  ...
  const [scaledIngredients, setScaledIngredients] = useState<ShoppingListItem[]>([]);
  ...
  useEffect(() => {
    const fetchRecipeDetails = async () => {
      try {
        setLoading(true);
        // For now, we'll simulate fetching a recipe from the LLM
        // In a real app, you'd call your local LLM API here
        const mockRecipe: LlmRecipeDetail = {
          id: recipeId,
          title: 'Mock LLM Recipe',
          description: 'This is a recipe generated by the local LLM for demonstration.',
          ...
          ingredients: [
            { 
              ...
              amount: 2, 
              unit: 'pieces', 
              original: '2 pieces chicken breast' 
            },
            { 
              name: 'Rice', 
              quantity: '1 cup',
          ...
          instructions: '1. Season chicken and cook in olive oil. 2. Cook rice separately. 3. Serve together.',
          summary: 'A simple and healthy chicken and rice dish.',
        };
        setRecipe(mockRecipe);
        setServings(mockRecipe.servings || 1);
        setScaledIngredients(mockRecipe.ingredients || []);
        setLoading(false);
      } catch (err) {
  ...
  const renderIngredients = (ingredients?: ShoppingListItem[]) => {
  ...
  const renderGroceryList = (ingredients?: ShoppingListItem[]) => {
    if (!ingredients || ingredients.length === 0) {
      return <Text style={[styles.noDataText, isDarkMode && styles.textLight]}>No ingredients information available</Text>;
    }
